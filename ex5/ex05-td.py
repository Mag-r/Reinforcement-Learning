...
def sarsa(env, alpha=0.1, gamma=0.9, epsilon=0.5, num_ep=int(1e4)):
    ...
    for i in range(num_ep):
        ...
        a = np.argmax(Q[s]) if np.random.rand() > epsilon else np.random.randint(env.action_space.n)
        counter=0
        while not done:
            # a = np.random.randint(env.action_space.n)
            s_, r, done, _ = env.step(a)
            a_ = np.argmax(Q[s_]) if np.random.rand() > epsilon else np.random.randint(env.action_space.n)
            Q[s, a] += alpha * (r + gamma * Q[s_, a_] - Q[s, a])
            s, a = s_, a_
            counter+=1
        episode_length.append(counter)
    return Q, episode_length
...
# print("Running sarsa...")
# Q, episode_length_sarsa = sarsa(env)
# plot_V(Q, env)
# plot_Q(Q, env)
# print_policy(Q, env)
# fig = plt.figure()
# episode_length = bn.move_mean(episode_length_sarsa, window=100, min_count=1)
# print(f"episode_length: {np.mean(episode_length_sarsa)}")
# plt.scatter(range(len(episode_length)), episode_length)
# plt.show()

print("\nRunning qlearning")
Q, episode_length_Q = qlearning(env)
plot_V(Q, env)
plot_Q(Q, env)
print_policy(Q, env)
fig = plt.figure()
episode_length = bn.move_mean(episode_length_Q, window=100, min_count=1)
plt.scatter(range(len(episode_length)), episode_length)
print(f"episode_length: {np.mean(episode_length_Q)}")
plt.show()
